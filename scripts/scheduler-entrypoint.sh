...
case "$1" in
  webserver)
    airflow db init
    echo "SUCCESS - airflow database initialization"
    sleep 2
    airflow users create -r Admin -u $AIRFLOW_ADMIN_USERNAME -e $AIRFLOW_ADMIN_EMAIL -f admin -l user -p $AIRFLOW_ADMIN_PASSWORD
